...
export default function UserBookings({}) {
  ...
  const getUpcomingEvents = async (before, pageStart, maxResults,  eventIds, setEvents, eventNum, setEventNum, setMoreEvents, setEventGroups) => {
    try {
      ...
      const response = await apiFetch('GET', `/api/user/bookings?${searchParams}`, null)
      console.log(response)
      const bookings = response.bookings
      const response_eventIds = []
      bookings.forEach(function (booking) {
        response_eventIds.push(booking.event_id)
      })

      if (pageStart === 0) {
        setEvents(response_eventIds)
        // Create event groups
        const groups_t = []
        var group_s = []
        var group_i = 0
        ...
        const eventIds_t = eventIds.concat(response_eventIds)
        ...
        console.log(groups_t)
        setEventGroups(groups_t)
        setEventNum(eventNum + response_eventIds)
      }
      setMoreEvents(response.num_results === (eventNum + response_eventIds))
